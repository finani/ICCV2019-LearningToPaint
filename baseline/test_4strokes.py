...
names = ['', '_notrans', '_round', '_triangle']
actors = ['checkpoints/actor.pkl', 'checkpoints/actor_notrans.pkl', 'checkpoints/actor_round.pkl', 'checkpoints/actor_triangle.pkl']
renderers = ['checkpoints/renderer.pkl', 'checkpoints/bezierwotrans.pkl', 'checkpoints/round.pkl', 'checkpoints/triangle.pkl']

for name, actor_pkl, renderer_pkl in zip(names, actors, renderers):
    print('make directory {}'.format('output' + name))
    ...
    Decoder = FCN()
    Decoder.load_state_dict(torch.load(renderer_pkl))
    ...
    def save_img(res, imgId, name, divide=False):
        ...
        cv2.imwrite('output' + name + '/generated' + str(imgId) + '.png', output)

    actor = ResNet(9, 18, 65) # action_bundle = 5, 65 = 5 * 13
    actor.load_state_dict(torch.load(actor_pkl))
    ...
    with torch.no_grad():
        ...
        if args.divide != 1:
            ...
            for i in range(max_step):
                ...
                for j in range(5):
                    save_img(res[j], imgid, name, True)
                    imgid += 1
